...
pub fn get_available_pairs() -> impl Future<Item=Vec<TradePair>, Error=failure::Error> {
    use std::io::Read;
    ...
    client::get("https://api.bitfinex.com/v1/symbols")
        .finish()
        .unwrap()
        .send()
        .map_err(|a| a.into())
        .map(|resp| {
            let pair_strs = resp.json::<Vec<String>>().wait().unwrap();
