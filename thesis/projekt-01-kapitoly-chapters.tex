...
The general goal is to have latency between receiving of data, and executing trads below one second.
\todo{Reword}
...
that is running on the server. The fronend is implemented in javascript, and its primary functions include interpreting system
data for user consumption, accepting user inputs and communicating with the \textit{backend}. The \textit{Backend} then
runs on the server, connects to the database, and usually provides some kind of a API, most commonly REST(\todo{Footnote})
...
async fn api_detail((req, id): (HttpRequest<State>, Path<i32>))
    ...
    require_cond!(strat.owner_id == base.auth.uid, "Not authorized");
...
The \textbf{BaseTemplateInfo} struct primarily contains the authorization information. This retrieval must be asynchronous,
...
\todo{History - used Server side templating, worked but was ugly and full of hacks}
\todo{Decided to move to React, re-implementation took 3 days compared to 2 or more weeks for sever side}
\todo{React - basics}
