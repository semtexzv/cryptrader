...
Our implementation heavily relies on this feature of Rust.

\todo{Actix built on top of this}
\todo{Mentin tokio lib}
...
\todo{Type erasure, message stored as a type name + json/Msgpack data}
\todo{If name not found, message not understood}
\todo{Fast, simple}

\subsubsection{Actor state}
\todo{Identifier tokens, currently int}
\todo{Channels , rx is provided as asynchronous reply, tx stored internally}
\todo{Rx is also a future, a lot of futures used}
...
This is an open source database which supports most of the modern SQL, and probably the only competitor to Oracle's offering
...
has methods to access individual prices described in the \todo{autoref OHLC definition}.
...
Acces to exchange APIs is implemented in the \verb|apis| library in the \verb|code/deps| directory. This library implements
primitives that could be shared between different exhange adapters, and contains a \verb|bitfinex| submodule that contains
...
Another approach considered was the use of distributed database. We chose to use ScyllaDB. This distributed database
is re-implementation of Cassandra DBMS. 

This issue could by fixed by sharding the
database with simple streaming replication, but that approach would only ensure readability of data during issue
with the master server, and would not allow writing of new data.
