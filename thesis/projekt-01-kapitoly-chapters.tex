...
\todo{Using these properties, we decided on actor model }
\todo{Actor model is heterogenous message passing based model of a distributed system}
\section{Actor model}
...
Thanks to this property of isolation, there are no necessary locks to ensure memory safety. Another benefit is,
...
in implementing types. However, the \trait{Sized} trait might not be so easy top comprehend.
...
The destruction of "Objects" is performed with the help of a trait system. If a type, implements the \trait{Drop} trait,
...
The \trait{Sync} trait is implemented for types that can be safely shared between threads(Reference to it can be shared between threads).
...
context returns an address of this actor. The address is in form of a \type{Addr<A>}struct, that provides methods for sending messages to
an actor.

\todo{Remove this ? Do we need it ?}
...
\todo{Cite C\#, or JS async-await}

Actors cant send message using only adresses. The \type{Addr} struct provides 2 primary methods for sending messages.
The \fun{send} method will send a message to another actor, and will return a future, that will eventuially resolve into
returned value. \fun{do\_send} will send a message, but will not return any value, providing fire-and-forget type of
communication. The only constraint placed on the message result type, is that the returned value can't contain any local
references.

The \fun{send} method is more interesting, because actor can react to message responses in asynchronous way.
A very common pattern is sending a message, then using several combinators from \trait{Future} trait to modify actor
...
\autoref{msg_example} showas precisely this pattern. The \type{Ingest} actor receives \type{IngestUpdate} message,
...
\todo{Diagram, Application, inside container image, then VM, contains Docker images, on top is kubernetes managing them}
...
\todo{Done as a autoscaling kubernetest hingy}
\todo{Load balancing part}
\todo{Dynbamically scaled number of workers}
\todo{Database latency, other issues, each worker will work on many tasks concurrently}

\todo{Diagram, Load balancer -> Worker container -> contains multiple worker actors -> backend database actors}
\todo{Communication diagram, Ready request,  work, work, work -> Finish, Finish, Finish ,}

\todo{Pool of evaluators}
\todo{Implement load balancing between them}
\todo{Let kubernetes take care of scaling the application and distributing evaluators}
